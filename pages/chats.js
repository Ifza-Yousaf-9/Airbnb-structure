...
const Chats = ({ allReservations, allChats }) => {
  console.log("allReservations: ", allReservations, "allChats: ", allChats);
          ...
          <div className="text-center">
            ...
            <div className="flex justify-around mt-4 w-full">
              ...
              <button
                ...
                className={`px-4 py-2 bg-red-500 text-white text-base font-medium rounded-md shadow-sm ${!showLoader &&
...
export async function getServerSideProps() {
  try {
    // Read the JSON file containing reservations data
    const reservationsFilePath = path.join(process.cwd(), 'sampleReservations.json');
    const reservationsData = fs.readFileSync(reservationsFilePath, 'utf-8');
    const sampleReservations = JSON.parse(reservationsData);

    // Read the JSON file containing chats data
    const chatsFilePath = path.join(process.cwd(), 'sampleChats.json',);
    const chatsData = fs.readFileSync(chatsFilePath, 'utf-8');
    const sampleChats = JSON.parse(chatsData);

    return {
      props: {
        allReservations: sampleReservations.sort(
